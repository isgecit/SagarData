...
namespace Attachment
{
    public class AttachmentCls
    {
        ...
        //Change- 27-08-2018 Insert Attachment error related change
        public DataTable InsertAttachmentdata()
        {
            List<SqlParameter> sqlParamater = new List<SqlParameter>();
            sqlParamater.Add(new SqlParameter("@RunningId", RunningId));
            sqlParamater.Add(new SqlParameter("@DocumentId", DocumentId));
            sqlParamater.Add(new SqlParameter("@AttachmentHandle", AttachmentHandle));
            sqlParamater.Add(new SqlParameter("@IndexValue", IndexValue));
            sqlParamater.Add(new SqlParameter("@PurposeCode", PurposeCode));
            sqlParamater.Add(new SqlParameter("@FileName", FileName));
        ...
        public DataTable GetExistingAttachment()
        {
            return SqlHelper.ExecuteDataset(Con, CommandType.Text, @"select [t_drid] ,[t_dcid] ,[t_hndl]
                                                                    ,[t_indx] ,[t_prcd] ,[t_fnam] ,[t_lbcd]
                                                                    ,[t_atby]  ,[t_aton] from ttcisg132200 where 
                                                                     t_hndl = '" + AttachmentHandle + "' and substring(t_indx,1,9) in ( '" + IndexValue + "') and t_fnam not in ('')").Tables[0];
        }

        public DataTable GetAllExistingAttachments()
        {
            return SqlHelper.ExecuteDataset(Con, CommandType.Text, @"
                                     ...
                                     SELECT [t_drid]
                                      ...
                                      WHERE t_hndl = '" + AttachmentHandle + "' and substring(t_indx,1,9) in ('" + IndexValue + "')").Tables[0]; //
        }
        ...
        public DataTable GetAttachments()
        {
            return SqlHelper.ExecuteDataset(Con, CommandType.Text, @"
                                     ...
                                     SELECT [t_drid]
                                      ...
                                      FROM [ttcisg132200]  ATH
                                      LEFT JOIN  ttcisg129200 AP ON AP.t_prcd=ATH.t_prcd
                                      WHERE t_hndl = '" + AttachmentHandle + "' and t_indx in ('" + IndexValue + "')").Tables[0]; //
        }
